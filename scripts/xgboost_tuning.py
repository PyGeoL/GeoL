...
def split_train_test_for_tuning(BASE_DIR_CITY, df_ua_fs, CITY_NAME, SIZE, METRIC, S, WS, C):
...
def modelfit(alg, X, y, useTrainCV=True, cv_folds=5, early_stopping_rounds=50, verbose=True):
    ...
    if useTrainCV:
        ...
        cvresult = xgb.cv(xgb_param, xgtrain, num_boost_round=alg.get_params()['n_estimators'], nfold=cv_folds,
                          metrics='rmse', early_stopping_rounds=early_stopping_rounds)  # verbose_eval=True)#show_progress=True)
...
def main(argv):
    ...
    parser.add_argument('-t', '--tuning',
                        help='Flag to perform parameters tuning',
                        action='store',
                        dest='tuning',
                        default=1,
                        type=int)

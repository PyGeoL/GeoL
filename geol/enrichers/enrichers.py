...
class LandUseErnicher(TessellationEnricher):
    ...
    def enrich(self, input, aggregation_level=0):
        ...
        landuse[constants.ITEM_COL] = landuse[constants.ITEM_COL].fillna(
            'Undefined')

        if aggregation_level == 1:
            for k, v in constants.urbanatlas_aggregation:
                for e in v:
                    landuse[constants.ITEM_COL].replace(
                        to_replace=k, value=e, inplace=True)

        gdf = landuse.to_crs({'init': 'epsg:4326'})[
            [constants.CITIES, constants.ITEM_COL, 'geometry']]

        # spatial join between grid and landuse
        df = gpd.sjoin(self._component.grid, landuse,
                       how='left', op='intersects')
        df = df.merge(landuse[['geometry']],
        ...
        df['percentage'] = df['area'] / \
            self._component.grid.loc[0].geometry.area
        ...
        r = pd.pivot_table(df, values='percentage', index=['cellID'], columns=[
                           constants.ITEM_COL]).reset_index()
